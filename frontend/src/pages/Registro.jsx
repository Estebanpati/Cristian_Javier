...
const EMPTY = { nombreCompleto: "", profesion: "", fechaNacimiento: "", idNivelAcademico: "", idBloque: "", idNivelResponsabilidad: "", pretensionSalarial: "" };
...
export default function Registro() {
  ...
  const handleSubmit = async () => {
    setSaving(true);
    try {
      await api.post("/personas", { ...form, idBloque: form.idBloque || null, pretensionSalarial: form.pretensionSalarial || null });
      setDone(true);
    } catch {
  ...
  const Field = ({ label, error, children, required }) => (
    <div className="form-group">
      <label className="form-label-clean">{label}{required && <span style={{ color: "var(--danger)", marginLeft: 3 }}>*</span>}</label>
      {children}
      {error && <div className="form-error"><i className="bi bi-exclamation-circle me-1"></i>{error}</div>}
    </div>
  );
        ...
        {!done ? (
          <>
              ...
              {STEPS.map((_, i) => (
                <div key={i} className="step-item">
                  <div className={`step-num ${i < step ? "done" : i === step ? "active" : ""}`}>
                    {i < step ? <i className="bi bi-check-lg" style={{ fontSize: "0.7rem" }}></i> : i + 1}
                  </div>
            ...
            <div className="step-labels">
              {STEPS.map((label, i) => (
                <div key={i} className={`step-label-item ${i === step ? "active" : i < step ? "done" : ""}`}>{label}</div>
              ))}
            </div>
            ...
            <div className="register-body">
              {step === 0 && (
                <>
                  <Field label="Nombre completo" required error={errors.nombreCompleto}>
                    <input className="form-control-clean" value={form.nombreCompleto} onChange={handleChange("nombreCompleto")} placeholder="Ej. Juan Carlos Pérez López" />
                  </Field>
                  <Field label="Profesión" error={errors.profesion}>
                    <input className="form-control-clean" value={form.profesion} onChange={handleChange("profesion")} placeholder="Ej. Licenciado en administración" />
                  </Field>
              ...
              {step === 2 && (
                <>
                  <div style={{ background: "#f8fafc", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "1rem", marginBottom: "1rem" }}>
                    <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: "0.75rem" }}>Resumen del registro</div>
                    {[
                  ...
                  {errors.submit && <div className="alert-error"><i className="bi bi-exclamation-triangle"></i>{errors.submit}</div>}
                </>
              )}
              ...
              <div style={{ display: "flex", gap: "0.75rem", marginTop: "1.25rem" }}>
                ...
                {step < 2 ? (
                  ...
                  <button className="btn-success-clean" onClick={handleSubmit} disabled={saving} style={{ flex: 1, justifyContent: "center" }}>
                    {saving ? <span className="spinner" style={{ width: 14, height: 14, borderWidth: 2 }}></span> : <i className="bi bi-send-check"></i>}
                    Enviar registro
                  </button>
                )}
              </div>
          ...
          <div className="register-body" style={{ textAlign: "center", paddingTop: "2rem", paddingBottom: "2rem" }}>
            ...
            <p style={{ color: "var(--text-muted)", fontSize: "0.9rem", marginBottom: "2rem" }}>
              <strong style={{ color: "var(--primary)" }}>{form.nombreCompleto}</strong> fue registrado correctamente en el sistema.
            </p>
            <div style={{ display: "flex", gap: "0.75rem", justifyContent: "center", flexWrap: "wrap" }}>
              <button className="btn-outline-clean" onClick={() => { setDone(false); setStep(0); setForm(EMPTY); }}>
